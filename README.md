...
- **Preprints:** the [arXiv Atom API](https://info.arxiv.org/help/api/user-manual.html). The surname search is filtered to Antonio Laface / A. Laface to exclude unrelated authors. All result pages and historical identifiers are supported. Matching arXiv identifiers, DOI, normalized title or an arXiv journal reference moves a manuscript into the published archive. Classification can lag if neither source has the publication record; the published archive remains accessible on the page. The main preprint list includes only first submissions within the last five years, using a rolling cutoff recalculated at each daily update. Older preprints remain in the metadata snapshot and are accessible through the link to all submissions on arXiv.
